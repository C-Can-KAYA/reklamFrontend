fetch('http://localhost:8080/reklam/findAll').then(response => response.json())
    .then(result => {
        ...
        for (var i = 0; result.length > i; i++) {
          ...
          cell4.innerHTML = "<a class='btn btn-sm btn-warning'href='reklamDetay.html?id="+result[i].id+"'>Detay</a><a class='btn btn-sm btn-info' href='reklamDetayDuzenle.html?id='#'>Düzenle</a><a class='btn btn-sm btn-primary' onclick='deleteReklam("+result[i].id+")' href='#'>Sil</a>";
        }
    }
    );
